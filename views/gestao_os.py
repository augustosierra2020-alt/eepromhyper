...
# ==========================================
# EXPORTAÇÃO EXCEL AGRUPADA (COM TOTAL E ESPAÇO)
# ==========================================
def gerar_excel_agrupado(df):
    """Gera um Excel formatado, agrupado por Flash Point, com total e espaçamento."""
    if df.empty:
        return io.BytesIO()

    df_sorted = df.copy()
    if "Flash Point" in df_sorted.columns:
        df_sorted["Flash Point"] = df_sorted["Flash Point"].fillna("SEM_FP")
        df_sorted = df_sorted.sort_values(by=["Flash Point"])
    ...
    # Estilo dos Cabeçalhos (Fundo Azul, Texto Branco e Negrito)
    ...
    for col_num, header in enumerate(headers, 1):
        ...
        ws.column_dimensions[get_column_letter(col_num)].width = 22  # Largura confortável
        
    bold_font = Font(bold=True)
    total_fill = PatternFill(start_color="FFF2CC", end_color="FFF2CC", fill_type="solid") # Fundo amarelo claro
    
    if "Flash Point" in df_sorted.columns:
        for fp, group in df_sorted.groupby("Flash Point"):
            # 1. Insere todas as linhas de serviço do cliente
            for _, row in group.iterrows():
                ws.append(row.tolist())
                
            # 2. Calcula a soma do valor
            soma = pd.to_numeric(group["Valor"], errors='coerce').sum()
            
            # 3. Cria a linha de TOTAL
            row_total = [""] * len(headers)
            if "Flash Point" in headers:
                row_total[headers.index("Flash Point")] = fp
            
            # Coloca a palavra TOTAL na Descrição ou Veículo
            if "Descrição" in headers:
                row_total[headers.index("Descrição")] = "TOTAL DO CLIENTE"
            elif "Veículo" in headers:
                row_total[headers.index("Veículo")] = "TOTAL DO CLIENTE"
                
            if "Valor" in headers:
                row_total[headers.index("Valor")] = soma
                
            ws.append(row_total)
            
            # 4. Aplica estilo na linha de Total
            current_row = ws.max_row
            for col_num in range(1, len(headers) + 1):
                cell = ws.cell(row=current_row, column=col_num)
                cell.font = bold_font
                cell.fill = total_fill
                if headers[col_num - 1] == "Valor":
                    cell.number_format = '"R$" #,##0.00'
                    
            # 5. Adiciona linha vazia para separar visualmente do próximo cliente
            ws.append([""] * len(headers))
    else:
        # Fallback de segurança se não houver coluna Flash Point
        for _, row in df_sorted.iterrows():
            ws.append(row.tolist())
            
    # Formatação de Moeda global para a coluna Valor
    if "Valor" in headers:
        val_idx = headers.index("Valor") + 1
        for r in range(2, ws.max_row + 1):
            if ws.cell(row=r, column=val_idx).value != "":
                ws.cell(row=r, column=val_idx).number_format = '"R$" #,##0.00'
            ...
            if arquivos_carregados:
                try:
                    ...
                    if dfs:
                        ...
                        # --- NOVO BOTÃO DE EXPORTAÇÃO DA PLANILHA FINAL ---
                        st.markdown("---")
                        st.markdown("### 📥 Exportar Planilha Processada (Organizada por Código FP)")
                        st.caption("Baixe a planilha completa com filtros, valores calculados, subtotais e blocos visuais por cliente.")
                        
                        excel_data = gerar_excel_agrupado(df_filtrado)
                        st.download_button(
                            label="📥 Baixar Planilha Final Oficial (.xlsx)",
                            data=excel_data,
                            file_name=f"FPF_Relatorio_Final_{datetime.now().strftime('%d_%m_%Y')}.xlsx",
