...
def limpar_descricao_os(desc):
    ...
    if match: return f"MOD OFF {match.group(0).replace(' ', '')}" if "MOD" in desc and "OFF" in desc else f"MOD {match.group(0).replace(' ', '')}" if "MOD" in desc else f"OFF {match.group(0).replace(' ', '')}" if "OFF" in desc else match.group(0).replace(" ", "")
...
def higienizar_valor_monetario_para_calculo(val):
    ...
    try: return float(val.replace(".", "").replace(",", ".") if "." in val and "," in val else val.replace(",", "."))
    except: return 0.0
...
def modificar_modelo_docx(modelo_bytes, flash_point, cliente_nome, city, contato, linhas_tabela, total_valor):
    ...
    if tabela_servicos:
        ...
        while len(tabela_servicos.rows) > len(linhas_validas) + 1: tabela_servicos._tbl.remove(tabela_servicos.rows[len(linhas_validas) + 1]._tr)
