...
BASE_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__))) if "views" in os.path.dirname(os.path.abspath(__file__)) else os.path.dirname(os.path.abspath(__file__))
LOGOS_DIR = os.path.join(BASE_DIR, "Logos")
DB_PATH = os.path.join(BASE_DIR, "eeprom_master.db")

# Tentativa de importação do core.db (Fallback seguro)
try:
    ...
    try:
        ...
        if row:
            ...
            result = {
                ...
                "escala": row[4], 
                "detalhes": row[5], 
                "graficos": [f[0] for f in cursor.fetchall()]
            }
    except Exception: 
        pass
    finally: 
...
def excluir_montadora_db(montadora):
    ...
    try:
        ...
        conn.commit()
        if os.path.exists(os.path.join(BASE_DIR, mont)): 
            shutil.rmtree(os.path.join(BASE_DIR, mont))
    except Exception: 
        pass
    finally: 
        conn.close()

# === RENDERIZAÇÃO DA TELA EEPROM ===
def render_eeprom_view():
    ...
    if st.session_state.montadora_selecionada == "":
        ...
        if st.button("⬅️ Voltar para Montadoras"):
            ...
            if escolha_modelo:
                ...
                with col_img:
                    if not dados_mapa or not dados_mapa["graficos"]: 
                        ...
                        st.caption("💡 *Clique no botão 'Ampliar Mapa' abaixo de cada imagem para explorar os gráficos na Engine HUD Dinâmica!*")
                        ...
                        def abrir_modal_zoom(foto_bytes, legenda_titulo):
                            ...
                            container.onmouseup = container.onmouseleave = (e) => {{
                                ...
                                if (Math.abs(velocityX) > 2 || Math.abs(velocityY) > 2) applyInertia();
                            }};
                            ...
                            with sub_cols[0]:
                                if idx < len(lista_fotos):
                                    try:
                                        ...
                                        st.error("Falha ao abrir imagem.")
    ...
    # === SEÇÃO DE CADASTRO E GERENCIAMENTO ===
    st.markdown("---")
                        ...
                        if v_manage_col1.button("💾 Salvar Alterações Veículo", type="primary"):
                            ...
                            if n_v_hig:
                                if n_v_hig != v_sel_edit:
                                    try: 
                                        shutil.rmtree(os.path.join(BASE_DIR, m_sel_v, v_sel_edit))
                                    except Exception: 
                                        pass
                                    conn = conectar_db()
                                    cursor = conn.cursor()
...
# Alias de compatibilidade para garantir importação em qualquer ponto do sistema
render_eeprom = render_eeprom_view
