...
def diagnostico_avancado_obd2(codigo, segmento="", montadora="", modelo="", ano=""):
    ...
    try:
        ...
        search_results = f"(Busca web indisponível: {e})"
        
    if not HF_TOKEN:
        return f"⚠️ **IA Offline:** Token ausente.\n\n**Dados da Web:**\n{search_results}"

    system_prompt = "Você é 'Chip', Mecânico Chefe Sênior. Forneça o laudo estruturado em: Significado, Descrição, Sintomas, Causas e Solução."
    user_prompt = f"Falha: {codigo}. Contexto: {segmento}|{montadora}|{modelo}|{ano}. Web:\n{search_results}"
    ...
    if client:
        try:
            ...
            return f"⚠️ **IA Offline:** Erro na API ({e}).\n\n**Dados da Web:**\n{search_results}"
...
def salvar_pesquisa_obd2(codigo, segmento, montadora, modelo, ano, descricao):
    ...
    try:
        ...
        st.toast(f"Erro ao salvar OBD2: {e}", icon="⚠️")
    finally:
        conn.close()
...
def render_obd2():
    ...
    with st.container(border=True):
        ...
        montadora_input = col_obd3.text_input("Montadora:", placeholder="Ex: Volkswagen")
        modelo_input = col_obd4.text_input("Modelo:", placeholder="Ex: Amarok V6")
        ano_input = col_obd5.text_input("Ano / Motor:", placeholder="Ex: 2021 / 3.0 TDI")
        
        if st.button("🚀 Iniciar Diagnóstico com IA", use_container_width=True, type="primary"):
            if codigo_input:
                with st.spinner(f"Analisando DTC {codigo_input}..."):
                ...
                st.warning("Digite um código de falha válido.")
            
    st.markdown("---")
    st.subheader("📚 Histórico de Consultas DTC")
    df_hist = carregar_historico_obd2()
    if not df_hist.empty:
        st.dataframe(df_hist, use_container_width=True)
    else: 
        st.info("Nenhuma pesquisa registrada até o momento.")